...
const projects = JSON.parse(
	await readFile(resolve(root, 'src/lib/corporate/projects.json'), 'utf8')
);
const css = (
	await Promise.all(
		['base', 'content', 'responsive', 'works'].map((name) =>
			readFile(resolve(root, `src/lib/corporate/${name}.css`), 'utf8')
		)
	)
).join('\n');
