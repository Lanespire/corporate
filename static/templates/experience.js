...
(() => {
  ...
  function animate(node, keyframes, options = {}) {
    ...
    const animation = node.animate(keyframes, { duration: 380, easing: 'cubic-bezier(.22,1,.36,1)', ...options });
  ...
  function motionChanged() {
    ...
    if (scene) { scene.style.setProperty('--scene-x', '0deg'); scene.style.setProperty('--scene-y', '0deg'); }
  }
  ...
  if (stage) $$('.package-layer', stage).forEach((node, index) => {
    const destination = getComputedStyle(node).transform;
    animate(node, [{ transform: 'translateY(30px) scale(.94)', opacity: .15 }, { transform: destination, opacity: 1 }], { duration: 800, delay: index * 100 });
  });
  ...
  function selectModule(button) {
    ...
    $$('.package-layer', stage).forEach(node => node.getAnimations().forEach(animation => animation.cancel()));
  ...
  moduleButtons.forEach((button, index) => {
    ...
    button.addEventListener('keydown', event => {
      ...
      if (event.key === 'ArrowLeft') destination = (index - 1 + moduleButtons.length) % moduleButtons.length;
      if (event.key === 'Home') destination = 0;
      if (event.key === 'End') destination = moduleButtons.length - 1;
      if (destination === undefined) return;
      event.preventDefault(); moduleButtons[destination].focus(); selectModule(moduleButtons[destination]);
    });
  });
  ...
  if (heroScene) {
    ...
    heroScene.addEventListener('pointerleave', () => {
      heroScene.style.setProperty('--scene-y', '0deg'); heroScene.style.setProperty('--scene-x', '0deg');
    });
  }
  ...
  if ('IntersectionObserver' in window) {
    const reveal = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        animate(entry.target, [{ transform: 'translateY(20px)', opacity: .5 }, { transform: 'translateY(0)', opacity: 1 }], { duration: 650 });
      });
    }, { threshold: .08 });
    $$('.reveal').forEach(node => { if (node.getBoundingClientRect().top > innerHeight) reveal.observe(node); });
  }
  ...
  if (lab) {
    ...
    function brandChanged(event) {
      ...
      $('#code-theme').textContent = $('input[name="brand-color"]:checked', lab).id.replace('color-', '');
      ...
      if (event?.target !== nameInput) {
        ...
        animate(visible, [{ opacity: .6, transform: 'translateY(7px)' }, { opacity: 1, transform: 'translateY(0)' }]);
      }
    }
  ...
  if (form) {
    form.inert = false;
    const inputs = { scratch: $('#scratch-input'), custom: $('#custom-input'), setup: $('#setup-input') };
    ...
    let copyText = '', announcementTimer = 0;
    ...
    function calculate(shouldAnnounce = true) {
      ...
      const invalid = Object.values(inputs).some(input => input.value.trim() === '' || !input.checkValidity() || !Number.isFinite(input.valueAsNumber));
      ...
      if (invalid) {
        ...
        ['scratch-total', 'template-total', 'savings-total'].forEach(id => $('#' + id).textContent = '—');
        ...
        $('#scratch-bar').style.width = '0%'; $('#template-bar').style.width = '0%';
      ...
      saving.replaceChildren(document.createTextNode((negative ? '−' : '') + man(Math.abs(difference))));
      const unit = document.createElement('small'); unit.textContent = '万円'; saving.append(unit);
      if (shouldAnnounce && previous !== saving.textContent) animate(saving, [{ transform: 'translateY(6px)', opacity: .7 }, { transform: 'translateY(0)', opacity: 1 }], { duration: 200 });
      $('#difference-label').textContent = negative ? 'この条件ではテンプレート側が高くなります' : difference === 0 ? 'この条件では初期費用が同額です' : 'この条件で、差額は';
      $('#saving-ratio').textContent = scratch === 0 ? '比較額が0円のため、割合は算出しません' : negative ? `入力額より${(Math.abs(difference) / scratch * 100).toLocaleString('ja-JP', { maximumFractionDigits: 1 })}%増` : `初期費用の${(difference / scratch * 100).toLocaleString('ja-JP', { maximumFractionDigits: 1 })}%相当`;
      $('#calc-breakdown').textContent = `${names[choice]} ${man(prices[choice])}万円 ＋ 追加改修${man(custom)}万円 ＋ 導入等${man(setup)}万円`;
      $('#cost-difference-copy').textContent = negative ? '価格だけでなく、再利用できる機能と必要な改修範囲を見直して比較してください。' : '共通機能を再利用し、独自の体験づくりに予算を回す選択肢です。';
      $$('.price-card').forEach(card => card.classList.toggle('is-comparing', card.dataset.plan === choice));
      ...
      $('.range-labels>span:last-child').textContent = `${Number(range.max).toLocaleString('ja-JP')}万円`;
      const sentence = `スクラッチ ${yen(scratch)}、テンプレート利用 ${yen(total)}、差額 ${negative ? 'マイナス' : ''}${man(Math.abs(difference))}万円。いずれも税別・仮定の試算です。`;
      if (shouldAnnounce) announcementTimer = setTimeout(() => { $('#calc-announcement').textContent = sentence; }, 220);
    ...
    $('#copy-estimate').addEventListener('click', async () => {
      ...
      try {
        ...
        field.className = 'calc-copy-field'; field.readOnly = true; field.value = copyText;
        field.setAttribute('aria-label', 'コピーする試算結果と注意事項');
        output.append(field); field.focus(); field.select();
