...
(() => {
	...
	const projects = JSON.parse($('#corporate-projects').textContent),
	...
	function setMenu(open, restore = false) {
		...
		$('.site-header .brand').inert = open;
		$('.header-contact').inert = open;
	...
	document.addEventListener('keydown', (e) => {
		...
		if (e.key === 'Tab') {
			const stops = [menuButton, ...$$('a,button', menu)],
				first = stops[0],
				last = stops.at(-1);
	...
	if ('IntersectionObserver' in window) {
		...
		['services', 'approach', 'works', 'company', 'contact'].forEach((id) =>
			navObserver.observe($('#' + id))
		);
	}
	...
	function setupReveal() {
		if (reduced || !('IntersectionObserver' in window)) {
			revealEls.forEach((n) => n.classList.remove('pending'));
			return;
		}
	...
	function render() {
		...
		$('.reading-progress span').style.transform =
			`scaleX(${clamp(scrollY / Math.max(1, doc.scrollHeight - innerHeight))})`;
	...
	if ('IntersectionObserver' in window)
		new IntersectionObserver((es) =>
			es.forEach((e) => hero.classList.toggle('scene-offscreen', !e.isIntersecting))
		).observe(hero);
	function updateMotion() {
		reduced = motionMedia.matches || manualReduced;
		...
		b.disabled = motionMedia.matches;
		$('.motion-label', b).textContent = motionMedia.matches
			? 'OS設定で動きを抑制'
			: reduced
				? '動きを再開'
				: '動きを抑える';
		if (reduced) {
			scene.style.setProperty('--scroll-shift', '0px');
	...
	function syncCarousel() {
		...
		const visible = cards
			.map((card, i) => ({ r: card.getBoundingClientRect(), i }))
			.filter(
				({ r }) => Math.min(r.right, rect.right) - Math.max(r.left, rect.left) > r.width * 0.5
			);
		if (visible.length) {
			const first = visible[0].i + 1,
				last = visible.at(-1).i + 1;
			$('#works-position').textContent =
				(first === last ? first : `${first}–${last}`) + ` / ${cards.length}`;
		}
	}
	function queueCarousel() {
		if (!carouselFrame) carouselFrame = requestAnimationFrame(syncCarousel);
	}
	function moveCarousel(direction) {
		const current = cards.reduce(
			(best, card, i) =>
				Math.abs(cardLeft(card) - track.scrollLeft) <
				Math.abs(cardLeft(cards[best]) - track.scrollLeft)
					? i
					: best,
			0
		);
	...
	function openDialog(d, trigger) {
		returnFocus.set(d, trigger || document.activeElement);
		d.showModal();
	...
	$$('dialog').forEach((d) => {
		...
		d.addEventListener('close', () => {
			...
			const f = returnFocus.get(d);
			if (f?.isConnected) f.focus({ preventScroll: true });
		});
		d.addEventListener('click', (e) => {
			if (e.target !== d || (d.id === 'contact-dialog' && sending)) return;
	...
	function image(p) {
		const box = element('div', undefined, 'product-media');
		box.style.setProperty('--media-bg', p.color);
		const label = p.kind === 'commissioned' ? '機能イメージ' : '公開紹介画像';
		const placeholder = element('div', undefined, 'image-fallback');
		placeholder.setAttribute('aria-hidden', 'true');
		placeholder.append(element('span', p.name), element('small', label));
		const img = new Image();
		img.alt = `${p.name}の${label}`;
		img.width = p.width;
		img.height = p.height;
		img.decoding = 'async';
		img.src = p.src;
	...
	$$('[data-open-project]').forEach((b) =>
		b.addEventListener('click', () => {
			const p = byId.get(b.dataset.openProject);
			if (!p) return;
			const label = p.engagementLabel || (p.kind === 'commissioned' ? '受託開発' : '自社サービス');
			const host = $('#project-dialog-content');
			host.replaceChildren(image(p));
			const body = element('div', undefined, 'dialog-body');
			body.append(
				element(
					'span',
					label,
					'work-kind ' + (p.kind === 'commissioned' ? 'commissioned' : 'owned')
				),
				element('p', p.tag, 'project-tag')
			);
			const title = element('h2', p.name);
			title.id = 'project-dialog-title';
			body.append(title, element('p', p.description, 'project-description'));
			const ul = element('ul');
			p.points.forEach((x) => ul.append(element('li', x)));
			body.append(ul, element('p', `開発領域：${p.roles.join(' / ')}`, 'project-roles'));
			if (p.url) {
				const link = element('a', `${p.name}の公開サイトへ ↗`, 'button primary');
				link.href = p.url;
				link.target = '_blank';
				...
				body.append(
					...
					element(
						'p',
						'掲載画像は公開紹介画像です。現在の機能・提供状況はリンク先の公開サイトをご確認ください。',
						'dialog-note'
					)
				);
			} else {
				body.append(
					element(
						'p',
						`${label}の実績です。企業名・システムURLは非公開としており、掲載画像は機能イメージです。`,
						'dialog-note'
					)
				);
				const link = element('a', '同様のシステム開発を相談する →', 'button primary');
				link.href = '#contact';
				link.addEventListener('click', () => {
					$('#project-dialog').close();
					$('#contact-form').elements.subject.value = 'Web・SaaS開発';
				});
				body.append(link);
			}
			host.append(body);
			openDialog($('#project-dialog'), b);
		})
	);
	$$('[data-open-privacy]').forEach((b) =>
		b.addEventListener('click', () => openDialog($('#privacy-dialog'), b))
	);
	...
	$$('[data-service]').forEach((a) =>
		a.addEventListener('click', () => {
			if ([...form.elements.subject.options].some((o) => o.value === a.dataset.service))
				form.elements.subject.value = a.dataset.service;
		})
	);
	message.addEventListener('input', () => {
		$('#message-count').textContent = `${message.value.length} / 1200`;
	...
	form.addEventListener('submit', (event) => {
		event.preventDefault();
		if (sending) return;
		for (const field of [form.elements.name, message])
			field.setCustomValidity(field.value.trim() ? '' : '空白以外の文字を入力してください。');
		if (!form.reportValidity()) return;
		const data = Object.fromEntries(new FormData(form));
		const pairs = [
			['会社名', data.company.trim() || '未入力'],
			['お名前', data.name.trim()],
			['メールアドレス', data.email.trim()],
			['ご相談分野', data.subject],
		...
		$('#contact-summary').replaceChildren(dl);
	...
	send.addEventListener('click', async () => {
		...
		try {
			...
			submission = null;
			form.reset();
			$('#message-count').textContent = '0 / 1200';
			$('#form-status').textContent =
				'お問い合わせを受け付けました。内容を確認のうえ、ご連絡いたします。';
			dialog.close();
			mailBody = '';
			mailSubject = '';
			$('#contact-summary').replaceChildren();
			$('#copy-fallback').value = '';
		} catch (error) {
			status.dataset.state = 'error';
			status.textContent =
				error.name === 'AbortError'
					? '送信結果を確認できませんでした。重複送信を避けるため、再送前に info@lanespire.com へお問い合わせください。'
					: '送信できませんでした。入力内容は保持しています。再度お試しいただくか、本文をコピーして info@lanespire.com へご連絡ください。';
		} finally {
			clearTimeout(timeout);
			sending = false;
			send.disabled = false;
			send.textContent = 'この内容で送信する →';
			$('.close-dialog', dialog).disabled = false;
		}
	});
	$('#contact-dialog').addEventListener('cancel', (event) => {
	...
	$('#copy-contact').addEventListener('click', async () => {
		...
		try {
			...
			$('.copy-status').textContent = '宛先・件名・本文をコピーしました。';
		} catch {
			...
			field.select();
			$('.copy-status').textContent =
				'下の欄を選択しています。コピーしてメールに貼り付けてください。';
		}
	});
	form.inert = false;
	window.addEventListener(
		'resize',
		() => {
			if (innerWidth > 800 && !menu.hidden) setMenu(false);
			queueCarousel();
			requestRender();
		},
		{ passive: true }
