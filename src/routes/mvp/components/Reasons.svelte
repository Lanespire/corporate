	...
	const reasons = [
		{
			icon: 'bolt',
			title: '最短7日で方向性が見える',
			body: '画面・操作感・優先機能を、資料ではなくブラウザ上で確認できます。'
		},
		{
			icon: 'award',
			title: '直接契約・再委託なし',
			body: '実装者と直接やり取りできる体制。下請け会社を経由せず、最初から最後まで同じチームが担当します。'
		},
		{
			icon: 'tag',
			title: '最初の範囲が明確',
			body: '初回は一番見せたい体験に集中。追加機能は次のスプリントで優先順位を決めます。'
		},
		{
			icon: 'sprint',
			title: 'スクラムで毎週前進',
			body: '毎週金曜にデモ。追加したい機能は優先順位をつけて次スプリントに回します。'
		},
		{
			icon: 'sparkles',
			title: 'AI活用で開発速度2〜3倍',
			body: 'コーディング・レビュー・テスト生成にAIを活用し、同じ予算を検証に必要な部分へ集中させます。'
		},
		{
			icon: 'handover',
			title: 'そのまま自走できる引き継ぎ',
			body: 'コード・設計・運用ドキュメントを毎週更新。内製化や別ベンダーへの移行も容易。'
		}
	];
</script>

<section id="reasons" class="reasons" aria-labelledby="reasons-heading">
	<div class="container">
		<header class="head">
			<span class="eyebrow">WHY LANESPIRE</span>
			<h2 id="reasons-heading">スタートアップに選ばれる理由</h2>
			<p class="lede">
					...
					<span class="icon" aria-hidden="true">
						{#if reason.icon === 'bolt'}
							<svg
								...
								stroke-linejoin="round"><path d="M13 2L4 14h7l-1 8 9-12h-7l1-8z" /></svg
							>
						{:else if reason.icon === 'award'}
							<svg
								viewBox="0 0 24 24"
								fill="none"
								stroke="currentColor"
								...
								><circle cx="12" cy="9" r="6" /><path d="M8.5 14L7 22l5-3 5 3-1.5-8" /></svg
							>
						{:else if reason.icon === 'tag'}
							<svg
								viewBox="0 0 24 24"
								fill="none"
								stroke="currentColor"
						...
						{:else if reason.icon === 'sprint'}
							<svg
								...
								stroke-linecap="round"
								stroke-linejoin="round"
								><path d="M21 12a9 9 0 11-3-6.7" /><path d="M21 4v5h-5" /></svg
							>
						{:else if reason.icon === 'sparkles'}
							<svg
								viewBox="0 0 24 24"
								fill="none"
								stroke="currentColor"
						...
						{:else}
							<svg
								...
								stroke-linecap="round"
								stroke-linejoin="round"
								><path d="M9 19l-5-5 5-5" /><path d="M4 14h11a5 5 0 005-5V5" /></svg
							>
						{/if}
					</span>
					<h3>{reason.title}</h3>
...
<style>
	...
	.card {
		...
		padding: var(--mvp-space-8);
		box-shadow: var(--mvp-shadow-xs);
		transition:
			transform var(--mvp-transition),
			box-shadow var(--mvp-transition),
			border-color var(--mvp-transition);
	}
	...
	.icon {
		...
		height: 48px;
		border-radius: 12px;
		background: var(--mvp-bg-soft-orange);
		color: var(--mvp-accent);
		margin-bottom: var(--mvp-space-5);
	}
