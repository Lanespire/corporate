...
export const GET: RequestHandler = () => {
	...
	const projects = portfolio.map((project) => {
		...
		if (!asset?.card?.src || !asset?.detail?.src)
			throw new Error(`Missing asset: ${project.id}. Run npm run prepare:corporate.`);
		return {
			...project,
			engagementLabel:
				project.engagementLabel ?? (project.kind === 'commissioned' ? '受託開発' : '自社サービス'),
...
<div class="work-body"><div class="work-meta"><span class="work-kind ${commissioned ? 'commissioned' : 'owned'}">${label}</span><span class="work-category">${escape(p.tag)}</span></div><h3>${name}</h3><p class="work-description">${escape(p.description)}</p><p class="work-roles">${p.roles.map(escape).join(' / ')}</p><div class="work-actions"><button type="button" class="work-detail js-only" data-open-project="${p.id}">詳細を見る${arrow}</button>${externalLink}</div></div>
	...
	const json = JSON.stringify(projects)
		...
		.replace(/\u2029/g, '\\u2029');
	const html = template
		.replace('<!-- WORK_CARDS -->', cards)
		.replace('<!-- PROJECT_DATA -->', json)
		.replace(
			'受託案件は企業名を非公開で掲載しています。',
			'業務委託・受託開発案件は企業名を非公開で掲載しています。'
		)
		.replace(
			'画像は既存の紹介素材を使用しています。',
			'企業名非公開の案件の画像は機能イメージです。'
		)
		.replace(
			'.works-track{scroll-snap-type:x mandatory}',
			'.product-media img{opacity:1!important}.works-track{scroll-snap-type:x mandatory}'
		);
