<script lang="ts">
	...
	const promises = [
		...
		{ icon: ShieldCheck, title: '無理な営業なし', text: '安心してご相談可能' }
	];
	...
	const concepts = [
		{
			icon: Sparkles,
			title: 'むずかしいAIではなく、\nやさしい仕組みを。',
			body: '特別な知識がなくても使えるUIで、誰もが成果を出せる仕組みをつくります。'
		},
		{
			icon: ClipboardCheck,
			title: '小さく始めて、\n確かに効果を。',
			body: 'まずは一部の業務から。効果を見ながら少しずつ、確実に広げていきます。'
		},
		{
			icon: LineChart,
			title: '定着・運用まで、\nしっかり伴走。',
			body: '導入して終わりにしません。現場に根づき、成果が続くまで伴走します。'
		}
	];
	...
	const flow = [
		{
			icon: UsersRound,
			title: 'ヒアリング・要件整理',
			body: '課題やゴールを整理し、優先度の高いテーマを特定。'
		},
		{
			icon: FilePenLine,
			title: '設計・プロトタイプ',
			body: '使いやすいUI/UXと最小構成のプロトタイプを作成。'
		},
		{
			icon: Code2,
			title: '開発・テスト',
			body: '必要な機能を開発し、品質を担保してリリース準備。'
		},
		{ icon: Rocket, title: 'リリース・導入支援', body: 'スムーズな導入と現場トレーニングを実施。' },
		{
			icon: LineChart,
			title: '運用・改善',
			body: 'データを活用して改善を継続。成果を最大化します。'
		}
	];

	const reasons = [
		{ icon: WandSparkles, title: '現場目線の提案', body: 'ユーザー体験を重視した設計' },
		{ icon: FilePenLine, title: '小さく始める導入', body: 'スモールスタートでリスクを最小化' },
		{ icon: ClipboardCheck, title: '透明性のある開発', body: '進捗・見積・リスクをオープンに共有' },
		{ icon: ShieldCheck, title: '高い技術力', body: 'AI・自動化・Web・SaaSまで対応' },
		{ icon: Star, title: '伴走型サポート', body: '長く付き合えるパートナーとして支援' }
	];
...
<div class="lp-shell">
	<header class="site-header">
		<button
			...
			type="button"
			aria-label="Lanespire トップ"
			on:click={() => goToSection('top')}
		>
		...
		<nav class="desktop-nav" aria-label="メインナビゲーション">
			<button type="button" on:click={() => goToSection('services')}>サービス</button>
	...
	<main id="top">
		<section class="hero-section">
			<div class="hero-copy">
				<p class="eyebrow">AI・業務自動化の開発パートナー</p>
				<h1>
					<span class="line"><strong>AI</strong>を、現場の人が</span>
					<span class="line">使える形にする。</span>
				</h1>
				<p class="lead">
					難しいAI用語より、毎日の業務がどう楽になるか。<br
					/>私たちは、AI・Web・業務自動化・SaaSで、現場の「困っている」に寄り添い、使いやすく、続く仕組みをつくります。
				</p>
				<div class="hero-buttons">
					<a class="primary" href="#contact"
						><CalendarDays size={19} />AI導入を相談する<ArrowRight size={18} /></a
					>
					<a class="secondary" href="#works">実績を見る<ArrowRight size={18} /></a>
				</div>
				<div class="trust-row">
					{#each promises as promise}
						...
						<div class="trust-card">
							<Icon size={30} />
							<p><strong>{promise.title}</strong><span>{promise.text}</span></p>
						</div>
					{/each}
				</div>
			</div>
			...
			<div class="hero-art">
				...
				<div class="floating-card chart-card">
					<span>今月の削減時間</span>
					<strong>32.5<small>時間</small></strong>
					<em>▲ 81%</em>
					<div class="chart-line"></div>
				</div>
				<div class="floating-card flow-card">
					<strong>自動化フロー</strong>
					<div class="mini-flow">
						<span>入力</span><b></b><span>処理</span><b></b><span>出力</span>
					</div>
				</div>
			</div>
		</section>
		...
		<section class="concept-section" id="concept">
			<h2>私たちの考え方 <span>| AIを、やさしく、わかりやすく。</span></h2>
			<div class="concept-grid">
				{#each concepts as concept, index}
					...
					{#if index < concepts.length - 1}<span class="concept-arrow">→</span>{/if}
				{/each}
			</div>
		</section>

		<section class="services-section" id="services">
			<h2 class="section-title">サービス <span>✦</span></h2>
		...
		<section class="flow-section" id="flow">
			...
			<p class="section-lead">最短で価値を届け、現場に定着するまで伴走します。</p>
			<div class="flow-grid">
		...
		<section class="reason-section">
			<h2 class="section-title">Lanespireが選ばれる理由</h2>
	...
	<footer class="footer">
		<div class="footer-brand">
			<button class="brand brand-button" type="button" on:click={() => goToSection('top')}
				><span class="brand-mark" aria-hidden="true"></span><span>Lanespire</span></button
			>
			<p>AI・Web・業務自動化・SaaS開発で、事業の成長に伴走する開発パートナー。</p>
			<small>© 2024 Lanespire Inc.</small>
		</div>
		<nav>
			<strong>サービス</strong>
...
<style>
	...
	.lp-shell {
		...
		--panel: #ffffff;
		background:
			radial-gradient(circle at 78% 4%, rgba(79, 197, 189, 0.2), transparent 24%),
			linear-gradient(180deg, #fffefa 0%, #fbfaf7 70%, #ffffff 100%);
		color: var(--ink);
		min-height: 100vh;
	}
	...
	.primary {
		background: linear-gradient(135deg, #ff893f, var(--orange));
		color: #fff;
		box-shadow: 0 12px 24px rgba(255, 122, 53, 0.22);
	}
	.secondary {
		background: #fff;
	...
	.hero-section {
		...
		display: grid;
		gap: 38px;
		grid-template-columns: minmax(560px, 0.94fr) minmax(600px, 1.06fr);
		min-height: 610px;
		padding: 36px 0 28px;
	}
	.eyebrow {
	...
	.floating-card {
		...
		border-radius: 10px;
		box-shadow: 0 16px 38px rgba(35, 62, 70, 0.1);
		display: none;
		position: absolute;
		z-index: 3;
	}
	...
	.footer {
		...
		display: grid;
		gap: 54px;
		grid-template-columns: 1.5fr 0.65fr 0.7fr 0.8fr 1fr;
		margin: 0 auto;
		max-width: 1400px;
	...
	/* ---------- visual direction: obsidian editorial ---------- */
	:global(body) {
		background: #0c100f;
		color: #f3f0e8;
		font-family:
			'Avenir Next', 'Hiragino Sans', 'Yu Gothic', 'Noto Sans JP', system-ui, sans-serif;
	}
	...
	.lp-shell {
		...
		--teal-strong: #52ceb1;
		--orange: #8be6ce;
		--line: rgba(201, 224, 215, 0.16);
		--panel: #151a18;
		--panel-strong: #1b2220;
		background:
			radial-gradient(circle at 82% 0%, rgba(83, 205, 176, 0.13), transparent 25rem),
			radial-gradient(circle at 4% 44%, rgba(113, 92, 65, 0.11), transparent 29rem),
			#0c100f;
		color: var(--ink);
		isolation: isolate;
		position: relative;
	...
	.site-header {
		...
		border-bottom: 1px solid rgba(201, 224, 215, 0.12);
	...
	.brand-mark {
		background: var(--teal);
		box-shadow: 0 0 0 5px rgba(139, 230, 206, 0.1), 0 0 28px rgba(139, 230, 206, 0.36);
		height: 22px;
		width: 22px;
	}
	...
	.primary {
		background: var(--teal);
		box-shadow: 0 12px 32px rgba(82, 206, 177, 0.16);
		color: #10201b;
	}

	.secondary {
		background: rgba(255, 255, 255, 0.025);
		border-color: rgba(201, 224, 215, 0.3);
		color: var(--ink);
	}

	.primary:hover,
	.secondary:hover {
		box-shadow: 0 16px 36px rgba(82, 206, 177, 0.2);
		transform: translateY(-3px);
	}
	...
	.secondary:hover {
		background: rgba(139, 230, 206, 0.08);
		border-color: var(--teal);
	}
	...
	.trust-card {
		background: transparent;
		border: 0;
		border-left: 1px solid rgba(201, 224, 215, 0.2);
		border-radius: 0;
		box-shadow: none;
		gap: 11px;
		min-height: 58px;
	...
	.hero-art {
		background: #e9e8e0;
		border: 1px solid rgba(231, 238, 231, 0.5);
		border-radius: 28px;
	...
	.soft-blob {
		background:
			radial-gradient(circle at 52% 42%, rgba(124, 220, 197, 0.32), transparent 37%),
			radial-gradient(circle at 70% 70%, rgba(244, 202, 128, 0.22), transparent 33%);
		display: block;
		filter: blur(8px);
		inset: 6% 3% 8%;
		position: absolute;
	...
	.floating-card {
		...
		border: 1px solid rgba(139, 230, 206, 0.34);
		border-radius: 9px;
	...
	.assistant-card i {
		background: rgba(139, 230, 206, 0.22);
	}
	...
	.flow-section,
	.reason-section {
		border-top: 1px solid rgba(201, 224, 215, 0.13);
		padding: 94px 0 34px;
	}
	...
	.service-card.purple:focus-visible {
		...
		border-color: rgba(139, 230, 206, 0.65);
		box-shadow: none;
	...
	.service-card ul {
		border-top: 1px solid rgba(201, 224, 215, 0.12);
		gap: 7px;
		margin: 18px 0 24px;
	...
	.service-card.cyan li::marker,
	.service-card.green li::marker,
	.service-card.blue li::marker,
	.service-card.orange li::marker,
	.service-card.purple li::marker {
		color: var(--teal) !important;
	}
	...
	.work-card.featured,
	.work-card:hover {
		border-color: rgba(139, 230, 206, 0.52);
	}
	...
	.work-modal {
		background: #151a18;
		border-color: rgba(201, 224, 215, 0.22);
		border-radius: 16px;
	...
	.modal-close {
		background: #151a18;
		border-color: rgba(201, 224, 215, 0.24);
		color: var(--ink);
	}

	.modal-visual {
		background:
			radial-gradient(circle at 22% 20%, rgba(139, 230, 206, 0.16), transparent 34%),
			#e9e8e0;
	}
	...
	.modal-result-band {
		background: #1b2220;
		border-color: rgba(201, 224, 215, 0.14);
	}
	...
	.service-detail-grid::before {
		background: #151a18;
		border-color: rgba(139, 230, 206, 0.42);
		box-shadow: none;
		content: '↓';
	}
	...
	.reason-grid article {
		border-left: 1px solid rgba(201, 224, 215, 0.14);
		padding: 4px 18px;
	}
	...
	.reason-grid :global(svg) {
		background: transparent;
		border-color: rgba(139, 230, 206, 0.38);
		color: var(--teal);
	}
	...
	.contact-hero {
		background:
			radial-gradient(circle at 90% 15%, rgba(139, 230, 206, 0.26), transparent 30%),
			#dfe8dd;
		border: 1px solid rgba(139, 230, 206, 0.38);
		border-radius: 18px;
		color: #13221d;
		grid-template-columns: 270px 1fr;
		min-height: 250px;
	...
	.contact-points span {
		background: rgba(19, 34, 29, 0.06);
		border-color: rgba(19, 34, 29, 0.14);
		color: #25483d;
	}
	...
	.contact-form textarea {
		background: #0e1312;
		border-color: rgba(201, 224, 215, 0.2);
		color: var(--ink);
	}
	...
	.contact-form textarea:focus {
		border-color: var(--teal);
		box-shadow: 0 0 0 4px rgba(139, 230, 206, 0.12);
	}
	...
	.contact-success {
		background: rgba(139, 230, 206, 0.09);
		border-color: rgba(139, 230, 206, 0.35);
		color: var(--ink);
	}
	...
	.form-link {
		border-color: rgba(201, 224, 215, 0.28);
		color: var(--ink);
	}
	...
	@media (max-width: 720px) {
		...
		.trust-card:first-child {
			border-left: 0;
			border-top: 1px solid rgba(201, 224, 215, 0.14);
			padding: 13px 0 0;
		}
